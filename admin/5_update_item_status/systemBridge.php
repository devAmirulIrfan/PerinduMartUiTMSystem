...
if(isset($_POST["load_payment"])){
 $sql = "SELECT * FROM 4_payment WHERE state = 'IN PROGRESS' AND payment_status = 'PAID'  ";
 load($sql);
}
if(isset($_POST["search_payment"])){
    $payment_id = $_POST["payment_id"];
    $sql = "SELECT * FROM 4_payment WHERE state = 'IN PROGRESS' AND payment_id LIKE '%$payment_id%' AND payment_status = 'PAID'  ";
    load($sql);  
}
...
if(isset($_POST["verify_order_status"])){
    $payment_id = $_POST["payment_id"];
    $sql = "UPDATE 4_payment SET state='ORDER COMPLETED' WHERE payment_id=$payment_id";
    connect()->query($sql);   
}
